...
비선형적 자료구조이며 부모 → 자식처럼 계층적인 관계를 가지고 있는 자료구조입니다. 

- 힙(Heap)을 구현하는 방법 중 하나가트리 입니다.
...
이진 트리를 순회할때 일반적으로 스택을 사용해 돌게 됩니다 그러나 함수 호출보다 포인터를 사용하는게 오버헤드가 적기 때문에 **포인터를 사용해 순회의 효율성을 높인 스레드 이진 트리**를 사용할 수 있습니다.
...
- 균형에 대해서는 특별히 제한하지 않습니다.
- **각 노드가 자식을 많이 갖게 하여 트리의 높이를 줄입**니다.
...
위 방법처럼 트리를 연속된 배열에 저장할 수 있습니다.

full binary tree 또는 perfect binary tree일 경우 낭비되는 공간이 없지만 그렇지 않다면 낭비되는 공간이 2^n에 비례해 늘어납니다.

이러한 이유로 이진 트리는 보통 연결 리스트로 구현합니다.

### 연결리스트로 저장

![](https://velog.velcdn.com/images/leequiett/post/cf5de146-0234-48e2-bf20-d66d0bf272a5/image.png)

위 그림과 같이 왼쪽 서브트리 포인터, 데이터, 오른쪽 서브트리 포인터로 구성합니다.
